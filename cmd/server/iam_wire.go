...
func buildIAMHandler(ctx context.Context, getenv func(string) string, opts iamHandlerOpts) (http.Handler, func()) {
	...
	routerDeps := httpapi.Deps{
		...
		// SIN-65276 — count master Origin/Referer CSRF rejections on
		// /metrics (labelled by stable OriginCSRFReason cause) so a
		// CSRF-probe campaign against the master operator surface is
		// alertable, not just grep-able in the master_csrf_rejected log.
		// Registered on the shared default registry, like the authz
		// counters above.
		MasterCSRFRejection: newMasterCSRFRejectionCounter(prometheus.DefaultRegisterer),
